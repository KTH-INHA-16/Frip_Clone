...
class DailySearchViewController: BaseViewController {

    private let jwt = UserDefaults.standard.string(forKey: "userJWT")!
    var sortString: [String] = ["엑티비티","배움","건강·뷰티","모임"]
    var dailyTitle: [String] = ["인기","신규"]
    var sort:Int = 0
    var titleIdx:Int = 0
    ...
    init(sort: Int,titleIdx:Int) {
        self.sort = sort
        self.titleIdx = titleIdx
        super.init(nibName: "DailySearchViewController", bundle: Bundle.main)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        headerLabel.text = "\(dailyTitle[titleIdx]) \(sortString[sort-1])"
        headerLabel.sizeToFit()
        SearchGetDataManager().getFripDailyVC(targetURL: URL(string: Constant.FRIP_CATEGORY)!, categoryIdx: sort, sort: titleIdx, header: jwt, vc: self)
    }
...
extension DailySearchViewController: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        SearchGetDataManager().getDailyFripDetail(targetURL: URL(string: Constant.ALL_FRIP)!, index: cell.idx, header: jwt, vc: self)
    }
}
